...
class RopeData(data.Dataset):
    def __init__(self, action_dir, state_cur_dir, state_fut_dir, feat_cur_dir, feat_fut_dir):
    ...
    def __getitem__(self, idx):
        ...
        state_cur = np.array(Image.open(self.state_cur_list[idx]).resize((128,128))).transpose(2,0,1).astype(np.float)
        state_fut = np.array(Image.open(self.state_fut_list[idx]).resize((128,128))).transpose(2,0,1).astype(np.float)
