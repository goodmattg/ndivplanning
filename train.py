...
for epoch in range(num_epochs):
    for i, inputs in enumerate(loader):
        ...
        action, state_cur, state_fut, feat_cur, feat_fut = (
            action.to(gpu_id).float(),
            state_cur.to(gpu_id).float(),
            state_fut.to(gpu_id).float(),
            feat_cur.to(gpu_id).float(),
            feat_fut.to(gpu_id).float(),
        )
        ...
        for _ in range(3):
            D_loss = nn.BCEWithLogitsLoss()(
                ...
                torch.ones(action.size(0)).to(gpu_id),
